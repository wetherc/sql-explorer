        ...
        fn deref(&self) -> &Self::Target {
            ...
            unsafe {
                if buf.value.is_null() || buf.length == 0 {
                    ...
                    slice::from_raw_parts(buf.value.cast(), buf.length as usize)
                }
            }
        }
    }
...
impl<'a> Deref for BufRef<'a> {
    ...
    fn deref(&self) -> &Self::Target {
        // GSSAPI gives a buffer with no address when it holds nothing, and
        // `from_raw_parts` may not take such an address. The release 0.11 of
        // this crate carries the same guard.
        unsafe {
