...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> AsyncRead for TlsPreloginWrapper<S> {
    fn poll_read(
        ...
        // Anything that the TLS library wrote is still in the buffer, and it
        // must reach the server before an answer can arrive.
        //
        // A TLS library that this crate drives itself flushes at the end of
        // each round of the handshake. The library of the operating system
        // does not: it writes its messages and then reads. Sending the buffer
        // here puts one whole round of the handshake into one packet, which is
        // what the server expects, and it sends that packet at the moment the
        // library begins to wait.
        if inner.wr_buf.len() > HEADER_BYTES {
