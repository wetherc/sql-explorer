...
import { useTabsStore, QueryTab } from '../tabs';
...
describe('tabs store', () => {
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    (uuidv4 as vi.Mock).mockImplementation(() => Math.random().toString()); // Simple mock for unique IDs
  });
  ...
  it('addTab should add a new tab and set it as active', () => {
    const store = useTabsStore();
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-1');
  ...
  it('addTab should add a new tab with provided content', () => {
    const store = useTabsStore();
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-2');
  ...
  it('closeTab should remove the specified tab', () => {
    const store = useTabsStore();
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-a');
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-b');
  ...
  it('closeTab should activate a neighboring tab if the active tab is closed', () => {
    ...
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-1');
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-2');
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-3');
  ...
  it('closeTab should set activeTabId to null if last tab is closed', () => {
    const store = useTabsStore();
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-only');
  ...
  it('setActiveTab should change the active tab', () => {
    const store = useTabsStore();
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-x');
    (uuidv4 as vi.Mock).mockReturnValueOnce('tab-y');
